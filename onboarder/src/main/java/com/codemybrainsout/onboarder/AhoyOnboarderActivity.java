...
public abstract class AhoyOnboarderActivity extends AppCompatActivity implements View.OnClickListener, ViewPager.OnPageChangeListener
{
    ...
    public void setOnboardPages(List<AhoyOnboarderCard> pages, List<OnAhoyListeners> listeners)
    {
        ...
        this.pages = pages;
        ahoyOnboarderAdapter = new AhoyOnboarderAdapter(pages, listeners, getSupportFragmentManager(), dpToPixels(0, this), typeface);
        mCardShadowTransformer = new ShadowTransformer(vpOnboarderPager, ahoyOnboarderAdapter);
        mCardShadowTransformer.enableScaling(true);
        vpOnboarderPager.setAdapter(ahoyOnboarderAdapter);
        vpOnboarderPager.setPageTransformer(false, mCardShadowTransformer);
...
//    }
    ...
    {
        ahoyOnboarderAdapter.getTextInputFragPage(vpOnboarderPager.getCurrentItem()).showAcceptIcon();
    }

    public void showEnterIcon()
    {
        ahoyOnboarderAdapter.getTextInputFragPage(vpOnboarderPager.getCurrentItem()).showEnterIcon();
    }
    ...
    public TextInputLayout goToPage(int position)
    {
        vpOnboarderPager.setCurrentItem(position);
...
//        else if(ahoyOnboarderAdapter.getCardTypeAt(position) == AhoyOnboarderCard.OnboardType.INTRO && position)
    ...
    public void showBtnNext(boolean visible)
    {
        if(visible)
