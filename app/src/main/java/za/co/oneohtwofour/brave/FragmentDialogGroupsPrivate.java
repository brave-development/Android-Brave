...
public class FragmentDialogGroupsPrivate extends DialogFragment
{
    ...
    public void onActivityCreated(Bundle savedInstanceState)
    {
        ...
        fragGroups = (FragmentGroups) getFragmentManager().findFragmentByTag("fragGroups");
    ...
    private void subscribeUserToGroup(ParseObject group)
    {
        ...
        for(int i = 0; i < groupSubbedUsers.length(); i++)
        {
            try
            {
                if(HomeActivity.currentUser.getObjectId().equals(groupSubbedUsers.getString(i)))
        ...
        if(!userAlreadySubbed)
        {
            ...
            //sub group to user
            HomeActivity.currentUser.addUnique("groups", group.getString("name"));
